    ...
    return `
      <div class="student-card" onclick="Router.go('students', '${student.id}')" data-student-name="${UI.escape(student.name.toLowerCase())}">
        ...
        <div class="student-card-info">
          ...
          <div class="student-card-group">${UI.escape(student.groupName)} • ${UI.escape(student.subject)}</div>
        </div>
        <div class="student-card-actions">
          ${parentInfo?.parentPhone ? `
            <button class="btn btn-icon btn-ghost" onclick="event.stopPropagation(); Notifications.callParent('${student.id}')" title="Ara">📞</button>
            <button class="btn btn-icon btn-ghost" onclick="event.stopPropagation(); Notifications.messageParent('${student.id}')" title="WhatsApp">💬</button>
          ` : ''}
        </div>
      </div>
    `;
    ...
    container.innerHTML = `
      <div class="page-container fade-in">
        ...
        <div class="card-glass" style="text-align: center; padding: var(--space-xl); margin-bottom: var(--space-lg);">
          ...
          <h2 style="font-size: var(--font-xl); font-weight: 800; margin-bottom: 4px;">${UI.escape(student.name)}</h2>
          <div style="color: var(--text-tertiary); font-size: var(--font-sm);">
            ${UI.escape(student.groupName)} • ${UI.escape(student.subject)} • ${student.day}
          </div>
        ...
        <div style="display: flex; gap: 8px; margin-bottom: var(--space-xl);">
          ${parentInfo?.parentPhone ? `
            <button class="btn btn-primary btn-block" onclick="event.stopPropagation(); Notifications.callParent('${student.id}')">📞 Ara</button>
            <button class="btn btn-success btn-block" onclick="event.stopPropagation(); Notifications.messageParent('${student.id}')">💬 WhatsApp</button>
          ` : `
